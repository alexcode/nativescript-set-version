...
const paths = {
  androidManifest: "./app/App_Resources/Android/src/main/AndroidManifest.xml",
...
function getIOSVersionInfo(versionText, customBuild) {
  let versionInfo = {
  ...
  try {
    ...
    const currentVersionCode = customBuild || +versionCodeParts[versionCodeParts.length - 1];
...
async function setIosApplicationVersion(versionText, customBuild) {
  const { version } = await getIOSVersionInfo(versionText, customBuild);
...
async function getAndroidVersionInfo(versionText, customBuild) {
  let versionInfo = {
    currentVersionCode: null,
    currentVersion: null,
    version: null,
  ...
  try {
    ...
    const currentVersionCode = customBuild || +gradle.android.defaultConfig.versionCode;
    const version = versionStringToVersion(
...
async function setAndroidApplicationVersion(versionText, customBuild) {
  const { version, versionCode } = await getAndroidVersionInfo(versionText, customBuild);
  if (versionCode) {
    display("");
...
async function changeIOSVersion(versionText, customBuild) {
  ...
  } else if (versionText === undefined || versionText === '' || versionText === 'code') {
    versionText = `${currentVersion.version.major}.${currentVersion.version.minor}.${currentVersion.version.patch}`;
  }

  await setIosApplicationVersion(versionText, customBuild);
}
async function changeAndroidVersion(versionText, customBuild) {
  ...
  await setAndroidApplicationVersion(versionText, customBuild);
}

const changeVersion = async () => {
  let versionText, platform, customBuild;
  if (process.argv.length > 4) {
    customBuild = process.argv[4];
    versionText = process.argv[3];
    platform = process.argv[2];
  } else {
    customBuild = process.argv[3];
    versionText = process.argv[2];
  }
  customBuild = parseInt(customBuild);

  if (!platform) {
    await changeAndroidVersion(versionText, customBuild);
    await changeIOSVersion(versionText, customBuild);
